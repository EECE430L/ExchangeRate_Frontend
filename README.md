...
Note: For accessibility, _react-aria-live_ was used, but is outdated and consequently not compatible with _React v18_. Thus, after running `npm install`, there will be an error when installing _react-aria-live_. For the sake of this project, this dependency can be installed using the following command: `npm install react-aria-live --legacy-peer-deps`.
...
| `enums`      | Contains enumerations used in the React app, such as one defining the _TransactionType_.                                                                                    |
